...
def sample_sharegpt_requests_with_ratio(
    ...
    # 过滤掉对话轮次少于 2 的对话
    origin_dataset = [data for data in dataset if len(data["conversations"]) >= 2]
    ...
    for i in range(len(dataset)):
        ...
        prompt = "Answer the following questions:\n" + "\n".join(
            [f"{j + 1}. {q}" for j, q in enumerate(inputs)]
        )
        ...
        if prompt_len < 50:
            short_requests.append(origin_dataset[i])
        elif prompt_len > 1000:
            long_requests.append(origin_dataset[i])
