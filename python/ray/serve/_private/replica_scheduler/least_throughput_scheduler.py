...
from ray.serve._private.constants import (
    ...
    RAY_SERVE_THROUGHPUT_RESPONSE_DEADLINE_S,
    RAY_SERVE_MAX_THROUGHPUT_RESPONSE_DEADLINE_S,
    SERVE_LOGGER_NAME,
)
from ray.serve._private.replica_scheduler.common import (
    PendingRequest,
    ReplicaQueueLengthCache,
    ReplicaThroughputCache,
)
...
class PowerOfTwoChoicesReplicaScheduler(ReplicaScheduler):
    ...
    is sent a control message to fetch its throughput.
    ...
    throughput_response_deadline_s = RAY_SERVE_THROUGHPUT_RESPONSE_DEADLINE_S
    max_throughput_response_deadline_s = RAY_SERVE_MAX_THROUGHPUT_RESPONSE_DEADLINE_S
    ...
    def __init__(
        ...
        use_replica_throughput_cache: bool = False,
        ...
        self._use_replica_throughput_cache = use_replica_throughput_cache
        ...
        self._replica_throughput_cache = ReplicaThroughputCache(
            get_curr_time_s=get_curr_time_s,
        )
    ...
    def update_replicas(self, replicas: List[RunningReplica]):
        ...
        self._event_loop.create_task(self._probe_throughputs(replicas_to_ping, 0))
    ...
    async def return_all_replicas(
        ...
        try:
            ...
            while True:
                ...
                yield [self._replicas[chosen_id] for chosen_id in candidate_replica_ids]

    async def _probe_throughputs(
        self,
        replicas: List[RunningReplica],
        backoff_index: int,
    ) -> List[Tuple[RunningReplica, Optional[int]]]:
        """Actively probe the throughput from each of the replicas.

        Sends an RPC to each replica to fetch its throughput, with a response deadline
        that increases exponentially in backoff.

        Returns a list of throughputs in the same order as the replicas passed in.
        ...
        This method also updates the local cache of replica throughputs according to
        the responses.
        """
        ...
        max_throughput_response_deadline_s = max(
            self.throughput_response_deadline_s,
            self.max_throughput_response_deadline_s,
        )

        try:
            throughput_response_deadline_s = min(
                self.throughput_response_deadline_s * (2**backoff_index),
                max_throughput_response_deadline_s,
            )
            ...
            throughput_response_deadline_s = max_throughput_response_deadline_s

        get_throughput_tasks = []
        for r in replicas:
            t = self._event_loop.create_task(
                r.get_throughput(deadline_s=throughput_response_deadline_s)
            )
            t.replica = r
            get_throughput_tasks.append(t)

        done, pending = await asyncio.wait(
            get_throughput_tasks,
            timeout=throughput_response_deadline_s,
        ...
        for t in pending:
            ...
            t.cancel()
            logger.warning(
                f"Failed to get throughput from {replica.replica_id} "
                f"within {throughput_response_deadline_s}s. If this happens repeatedly "
        ...
        for t in done:
            ...
            if t.exception() is not None:
                result.append((replica, None))
                msg = (
                    "Failed to fetch throughput for "
    ...
    async def select_from_candidate_replicas(
        ...
        The throughput for each replica is first looked up in the local cache. If not
        ...
        if self._use_replica_throughput_cache:
            ...
            for r in candidates:
                ...
                elif throughput < lowest_throughput:
        ...
        if chosen_replica_id is None:
            for r, throughput in await self._probe_throughputs(
                not_in_cache,
                backoff_index,
            ):
                if throughput is None:
                ...
                if throughput < r.max_ongoing_requests and throughput < lowest_throughput:
                    lowest_throughput = throughput
                    chosen_replica_id = r.replica_id
            ...
            self._event_loop.create_task(
                self._probe_throughputs(not_in_cache, backoff_index)
            )
    ...
    async def fulfill_pending_requests(self):
        ...
        try:
            while len(self._scheduling_tasks) <= self.target_num_scheduling_tasks:
                ...
                # Choose all replicas to schedule the request.
                async for candidates in self.return_all_replicas():
                    ...
                    # Choose the best replica from these candidates.
